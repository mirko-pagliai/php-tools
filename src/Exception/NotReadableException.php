...
class NotReadableException extends FileException
{
    ...
    {
        parent::__construct($message ?: ($path ? sprintf('Filename `%s` is not readable', Filesystem::instance()->rtr($path)) : 'Filename is not readable'), $code, $severity, $filename, $lineno, $previous, $path);
    }
}
